...
SMODS.Joker {
    ...
    atlas = 'jokers',
    pos = { x = 0, y = 3 },
    config = { extra = { amp = 0.1, dollars = 4 } },
    loc_vars = function(self, info_queue, card)
        return { vars = { card.ability.extra.amp, card.ability.extra.dollars, card.ability.extra.amp * math.max(math.floor(((G.GAME.dollars or 0) + (G.GAME.dollar_buffer or 0)) / card.ability.extra.dollars), 0) } }
    end,
    calculate = function(self, card, context)
        if context.joker_main then
            return {
                amp = card.ability.extra.amp * math.floor(((G.GAME.dollars or 0) + (G.GAME.dollar_buffer or 0)) / card.ability.extra.dollars)
            }
        end
    end,
}
